...
6. **TDD First**: 必须定义如何生成单元测试和每个单元测试必须包含对请求边界,正确/错误路径的完整用例
7. **并行依赖**: 必须包含前置依赖任务,如此可以控制任务的并发度
...
委托任务到子代理, 规划开发阶段, 并行生成每个阶段的开发计划文件, 其中必须包含
...
{}
加载 Skill ddd-audit, 执行完成后调用 Skill ddd-audit
"#,
            PREPARE_PROMPT
        ))
    }
